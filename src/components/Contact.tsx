import styles from './Contact.module.css'

const WhatsappIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
)

const EmailIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <rect x="2" y="4" width="20" height="16" rx="2"/>
    <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
  </svg>
)

const InstaIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <rect x="2" y="2" width="20" height="20" rx="5"/>
    <circle cx="12" cy="12" r="4"/>
    <circle cx="17.5" cy="6.5" r="0.8" fill="currentColor" stroke="none"/>
  </svg>
)

export default function Contact() {
  return (
    <section id="contato" className={styles.section}>
      <div className={styles.grid}>
        <div className={`${styles.left} reveal`}>
          <p className={styles.label}>Contato</p>
          <h2 className={styles.title}>
            Fale<br />com nossa<br /><em>equipe</em>
          </h2>
          <p className={styles.desc}>
            Agende uma consulta e descubra como podemos proteger o que importa para você.
          </p>
          <div className={styles.info}>
            <a href="https://wa.me/5521979629991" target="_blank" rel="noreferrer" className={styles.infoItem}>
              <div className={styles.infoIcon}><WhatsappIcon /></div>
              <div>
                <p className={styles.infoLabel}>WhatsApp</p>
                <p className={styles.infoValue}>(21) 97962-9991</p>
              </div>
            </a>
            <a href="mailto:contato@opusgrupo.com" className={styles.infoItem}>
              <div className={styles.infoIcon}><EmailIcon /></div>
              <div>
                <p className={styles.infoLabel}>E-mail</p>
                <p className={styles.infoValue}>contato@opusgrupo.com</p>
              </div>
            </a>
            <a href="https://instagram.com/opusgrupo" target="_blank" rel="noreferrer" className={styles.infoItem}>
              <div className={styles.infoIcon}><InstaIcon /></div>
              <div>
                <p className={styles.infoLabel}>Instagram</p>
                <p className={styles.infoValue}>@opusgrupo</p>
              </div>
            </a>
          </div>
        </div>
        <div className="reveal" style={{ transitionDelay: '0.2s' }}>
          <form className={styles.form} noValidate>
            <div className={styles.row}>
              <div className={styles.group}>
                <label className={styles.formLabel} htmlFor="nome">Nome completo</label>
                <input className={styles.input} id="nome" type="text" placeholder="Seu nome" />
              </div>
              <div className={styles.group}>
                <label className={styles.formLabel} htmlFor="tel">Telefone</label>
                <input className={styles.input} id="tel" type="tel" placeholder="(21) 99999-9999" />
              </div>
            </div>
            <div className={styles.group}>
              <label className={styles.formLabel} htmlFor="email">E-mail</label>
              <input className={styles.input} id="email" type="email" placeholder="seu@email.com" />
            </div>
            <div className={styles.group}>
              <label className={styles.formLabel} htmlFor="msg">Mensagem</label>
              <textarea className={styles.textarea} id="msg" rows={5} placeholder="Descreva brevemente sua situação..." />
            </div>
            <button type="submit" className={styles.btn}>
              Enviar mensagem
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/>
              </svg>
            </button>
          </form>
        </div>
      </div>
    </section>
  )
}
